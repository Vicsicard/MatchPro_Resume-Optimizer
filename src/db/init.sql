...
-- Also create a trigger for auth.users to handle Supabase auth signups
DROP TRIGGER IF EXISTS on_supabase_auth_user_created ON auth.users;
CREATE TRIGGER on_supabase_auth_user_created
    AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user();

-- Update existing users to have 1 credit if they have less
